...
CLASS_GENERICSR  :typing.Callable[[int],str] = lambda n: '<{0}, {1}>'                  .format(EXC_TYPE, ', '.join(map(ARG_TYPE, range(n)))) if n > 0 else '<{0}>'                  .format(EXC_TYPE)
CLASS_GENERICS   :typing.Callable[[int],str] = lambda n: '<{0} extends Exception, {1}>'.format(EXC_TYPE, ', '.join(map(ARG_TYPE, range(n)))) if n > 0 else '<{0} extends Exception>'.format(EXC_TYPE)
    ...
    for i in range(1+N):
        ...
        class_code:str          = '\n'.join((
            ...
            textwrap.indent('\n'.join((
                ...
                ' */',
                'public void {method}({args}) throws {exct};'.format(method=METHOD_NAME, args=', '.join(map(ARG, range(i))), exct=EXC_TYPE),
            ...
            *(textwrap.indent('\n'.join((
                ...
                ' * @throws {0} generic error'.format(EXC_TYPE),
                ' */',
                f'default public void {METHOD_NAME_DEPRECATED}({', '.join(map(ARG, range(i)))}) throws {EXC_TYPE} {{{METHOD_NAME}({','.join(map(ARG_NAME, range(i)))});}}',
