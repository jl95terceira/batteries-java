package jl95.lang.primitive;

import jl95.util.DataClass;

import static jl95.lang.SuperPowers.I;
/**
 * A pointer to a value of type int. This is useful for simulating pass-by-reference in Java, or for creating mutable references to values.
 */
public class IntPointer extends DataClass {

    public interface Applier{
    
        int apply(int currentValue);
    }
    
    private int value;

    public IntPointer(int value) {this.value = value;}

    @Override protected Iterable<?> data() {
        return I(value);
    }

    public int get() {return value;}
    public void set(int x) {value = x;}
    public void apply(Applier f) {
        set(f.apply(get()));
    }
}
