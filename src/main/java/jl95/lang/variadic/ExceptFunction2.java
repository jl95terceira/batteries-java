package jl95.lang.variadic;

/**
 * An interface for a function that takes 2 arguments. <br/>
 * @param <A1> type of argument nr 1
 * @param <A2> type of argument nr 2
 * @param <R> type of return
 * @param <E> type of possible exception
 */
public interface ExceptFunction2<R, E extends Exception, A1, A2> {


    /**
     * interface method
     * @param a1 argument nr 1
     * @param a2 argument nr 2
     * @return whatever
     * @throws E generic error
     */
    public R apply(A1 a1, A2 a2) throws E;
    /**
     * interface method
     * @param a1 argument nr 1
     * @param a2 argument nr 2
     * @return whatever
     * @throws E generic error
     */
    default public R call(A1 a1, A2 a2) throws E { return apply(a1,a2);}
}